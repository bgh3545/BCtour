...
public class BCLoginController {
	...
	@PostMapping("/BClogin")
	public String login(BCUser user, boolean rememberId, HttpServletResponse response, HttpServletRequest request, String toURL) throws Exception{
		System.out.println("id=" + user.getId());
		System.out.println("pwd=" + user.getPwd());
		System.out.println("rememberId=" + rememberId);
		//1. id와 pwd 일치하는지 확인
		//2-1. 일치하지 않으면 loginForm이동
		if(!loginCheck(user.getId(), user.getPwd())) {
	...
	private boolean loginCheck(String id, String pwd) throws Exception {
		...
		BCUser user = bcUserDao.selectUser(id);
