	...
	public DBUser selectUser(String id) {
		...
		try {
			...
			if(rs.next()) {
				...
				user.setBirth(new Date(rs.getDate(5).getTime())); // 날짜 정보
				user.setSns(rs.getString(6));
				user.setReg_date(new Date(rs.getTimestamp(7).getTime())); // 날짜 + 시간 정보
			}
		} catch (SQLException e) {
			return null;
		} finally {
...
//		String sql = "update member set pwd=?,name=?,email=?,birth=?,sns=?,reg_date=? where id=?";
		...
		try (
			...
			PreparedStatement pstmt = conn.prepareStatement(sql); 	// AutoCloseable로 구현한 객체 이므로 close자동 호출
		) {
			...
			pstmt.setTimestamp(6, new java.sql.Timestamp(user.getReg_date().getTime())); // 날짜 + 시간
